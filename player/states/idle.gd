...
func enter() -> void: # was passiert wenn state betreten
	player.animation_player.play("Idle") #animation abspielen
	pass
...
func handle_input( event : InputEvent ) -> PlayerState: # was passiert wenn input gedrückt/losgelassen wird
	if event.is_action_pressed("jump"): # wenn jump-button gedrückt, zu jump wechseln
		return jump
	return next_state

func process(_delta: float) -> PlayerState: # was jeden tick passiert
	if player.direction.x != 0: # prüfen ob spieler sich bewegt, zu run wechseln
		return run
	return next_state

func physics_process(_delta: float) -> PlayerState: # was jeden physics-tick passiert
	player.velocity.x = 0 # x-geschw. 0 setzen
